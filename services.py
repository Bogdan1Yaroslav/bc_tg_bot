...
def best_change_request(exchanger_id: str) -> str:
    """
    ...
    try:
        ...
        if my_request.status_code == 200:
            ...
            if exchanger_id in exchangers_dict:
                ...
                output = f"Курс {exchanger_id}: {requested_value}\n" \
                         f"Курс лидера {max_value_exchanger}: {max_value}\n" \
                         f"Разница между курсами: {result}"

            else:
                output = 'Данного ID нет на сайте.\nПопробуйте снова.'

        else:
            output = 'Упс! Неизвестная ошибка.\nПопробуйте позже.'

    except:
        output = 'Ошибка соединения.\nПопробуйте позже.'
